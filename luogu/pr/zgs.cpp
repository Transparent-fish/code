    ...
    if ((a[now].name == "MP" || a[now].name == "ZP") && y == 1) {
      for (int i = 1; i <= a[now].cnt; i++) {
        if (a[now].card[i] == 'J' && a[now].vis[i] == 0) {
          a[now].vis[i] = 1;
          if (a[now].is_jump == 0) a[now].is_jump = 1, a[now].leifan = 0;
          return !use_J(now, 2);
        }
      }
      ...
      for (int i = 1; i <= a[now].cnt; i++) {
        if (a[now].card[i] == 'J' && a[now].vis[i] == 0) {
          a[now].vis[i] = 1;
          if (a[now].is_jump == 0) a[now].is_jump = 2, a[now].leifan = 0;
          return !use_J(now, 1);
        }
      }
    }
...
bool Use_F(int x) {  // x用 已完成
  ...
  } else if (a[x].name == "MP") {  // MP targets class FP or jumped FP
    for (int i = a[x].nex; i != x; i = a[i].nex) {
      if (a[i].leifan || a[i].is_jump == 2) {
        now = i;
        break;
      }
    }
    if (now == 0) return 0;
    bool can_use_J = use_J(x, 2);
    if (can_use_J == 0) {
      implement_F(x, now);
      return 1;
    }
...
bool Use_N(int x) {  // x用 已完成
  for (int i = a[x].nex; i != x; i = a[i].nex) {
    bool is_use_kill = 0, skip = 0;
    if (a[i].is_jump == 1) {
      int use_J_y = (a[i].is_jump == 2 ? 2 : 1);
      if (use_J(x, use_J_y)) skip = 1;
    }
    if (skip) continue;
    for (int j = 1; j <= a[i].cnt; j++) {
    ...
    if (is_use_kill == 0) {
      a[i].hp--;
      if (i == 1 && a[x].is_jump == 0) a[x].leifan = 1;
      if (a[i].hp <= 0) Die(i, x);
...
bool Use_W(int x) {  // 已完成
  for (int i = a[x].nex; i != x; i = a[i].nex) {
    ...
    if (is_use_kill == 0) {
      a[i].hp--;
      if (i == 1 && a[x].is_jump == 0 && a[x].name != "MP") a[x].leifan = 1;
      if (a[i].hp <= 0) Die(i, x);
      if (who_win != -1) return 1;
    }
  }
